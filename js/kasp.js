function analysis() {
    ...
    for (let n = 0; n < n_seq; n++) {
        ...
        let resl1 = AsPCR(snp, seqs[n], maskrepeats, minlc, minlen, maxlen, mintm, maxtm, primeroverlap, ctconvert, ends3);
        resultarea1 += resl1.resultarea1;
        resultarea2 += resl1.resultarea2;

        //REVERSE DIRECTION
        let resl2 = AsPCR(snp, ComplementDNA3(seqs[n]), maskrepeats, minlc, minlen, maxlen, mintm, maxtm, primeroverlap, ctconvert, ends3);
        resultarea2 += "\n";
...
function ProbeDesignForward(snp, s1, s2, pol, minlen, maxlen, mintm, nm) {
    const salt = 0.055;
    const Mg_M = 0.001;
    const p_mkM = 0.2;
    const min3 = 6;
    ...
    for (let i = 0; i < n; i++) {
        for (let j = i; j < n; j++) {
            if (DimerLook2(fpr[i], fpr[j], min3) > 0) {
                flc[j] = 0;
                flc[i] = 0;
            }
        }
                ...
                if (q1 === 0) {
                    ...
                    if (nprlist > 0 && q1 === 0) {
                        for (let j = 0; j < nprlist; j++) {
                            if (DimerLook2(s1, prlist[j], min3) > 0) {
                                q1 = 1; break;
                            }
                        }
                    }
...
function AsPCR(snp, inseq, maskrepeats, minlc, minlen, maxlen, mintm, maxtm, primeroverlap, ctconvert, ends3) {
    ...
    let result = ProbeDesignForward(snp, s1, s2, st, minlen, maxlen, mintm, "F_");
    let fpr = result.fpr;
    let fpn = result.fpn;
    let ftm = result.ftm;
    let flc = result.flc;
    ...
    if (fn > -1 && rn > -1) {
        ...
        for (let r = rn; r > -1; r--) {
            ...
            if (x3asp < xr) {
                ...
                let ta = Tm77(sq.substring(fpx[0], xr + rpr[r].length - 1), KMg_M, 0);
                for (let f = 0; f < fn; f++) {
